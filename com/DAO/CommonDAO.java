...
public class CommonDAO
{
	...
	public static ResultSet getFilesByGrpName(String usercode) 
	{
		try
		{
			DAO dao=DAO.getInstance();
			connection=dao.connector();
			statement = connection.createStatement();
	resultSet = statement.executeQuery("select * from m_transaction where u_code='"+usercode+"'");
			String sql="select * from m_transaction where u_code='"+usercode+"'";
			System.out.println("Sql query is>>>>>>>>>>>>>>>>"+sql);
			
			/*resultSet = statement.executeQuery("select f_name from m_file_upload where (select f_code from m_transaction where u_code='"+usercode+"')");
			String sql="	select f_name from m_file_upload where (select f_code from m_transaction where u_code='"+usercode+"')";
			System.out.println("Sql query is>>>>>>>>>>>>>>>>"+sql);
		*/
		...
		{
			System.out.println("Opp's Error is in CommonDAO-getFilesByGrpName()....."+e);
		}
		return resultSet;
	}
	...
	public static ResultSet getFiles1(String id) 
	{
		String type="yes";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			resultSet = statement.executeQuery("select * from m_transaction where u_code='"+id+"' and t_status='"+type+"'");
		}
		catch(Exception e)
		{
			System.out.println("Opp's Error is in CommonDAO-getUsers()....."+e);
	...
	public static String getFileNo(String id) 
	{
		String fname="";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			resultSet = statement.executeQuery("select f_code from m_transaction where t_no='"+id+"'");
			while(resultSet.next())
			{
				fname=resultSet.getString(1);
			}
	...
	public static String getFileNoname(String id) 
	{
		String fname="";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			resultSet = statement.executeQuery("select f_name,genesis_blk,hash_tag,blkid,random_no from m_file_upload where f_code='"+id+"'");
			while(resultSet.next())
			{
	...
	public static String getFileNoname1(String id) 
	{
		String fname="";
		try
		{
	...
	//genesis blockid//
	public static String previousblkid(String fno)
	{
		String name="";
		try
		{
	...
	//genesis block//
	public static String previousblk(String previousblk)
	{
		String name="";
		try
		{
	...
	public static String getMailID1(String userid) 
	{
		String email="";
		try
		{
			...
			connection=dao.connector();
			statement = connection.createStatement();
			resultSet = statement.executeQuery("select u_email from m_user where u_code='"+userid+"'");
			while(resultSet.next())
			{
				email=resultSet.getString(1);
			}
